...
describe("Stage 5 initial schema migrations", () => {
  ...
  it("applies, verifies constraints, rolls back to empty, and reapplies", async () => {
    ...
    await sequelize.query("INSERT INTO shipments (id, shipment_number, source_type, source_id, order_id, pickup_warehouse_id, weight_grams, length_cm, width_cm, height_cm) VALUES (8301, 'SHP-008301', 'order', :order, :order, '24', 500, 10, 10, 10)", { replacements: { order: ids.order } });
    await expectSqlRejected("INSERT INTO shipments (id, shipment_number, source_type, source_id, order_id, pickup_warehouse_id, weight_grams, length_cm, width_cm, height_cm) VALUES (8302, 'SHP-008302', 'order', :order, :order, '24', 500, 10, 10, 10)", { order: ids.order });
