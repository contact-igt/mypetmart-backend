...
const table = getInitialSchemaTable("product_content_blocks");

export async function up({ context }: MigrationArguments): Promise<void> {
  await createInitialSchemaTable(context, table);

  // Same id_sequences backfill as 048-create-product-features.ts / 051-create-product-specifications.ts:
  // this table is created after 022-create-id-sequences.ts ran, so it needs its own seed row.
