...
const table = getInitialSchemaTable("shipment_tracking_events");
...
export async function down({ context }: MigrationArguments): Promise<void> {
  await context.sequelize.query("DELETE FROM `id_sequences` WHERE `sequence_name` = ?", { replacements: [table.tableName], type: QueryTypes.DELETE });
