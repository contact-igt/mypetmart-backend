...
const table = getInitialSchemaTable("notification_log");

export async function up({ context }: MigrationArguments): Promise<void> {
  await createInitialSchemaTable(context, table);

  // Same id_sequences backfill as 044-create-newsletter-subscribers.ts: this
  // table is created after 022-create-id-sequences.ts ran, so it needs its
  // own seed row.
