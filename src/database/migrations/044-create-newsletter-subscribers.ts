...
const table = getInitialSchemaTable("newsletter_subscribers");

export async function up({ context }: MigrationArguments): Promise<void> {
  await createInitialSchemaTable(context, table);

  // Same id_sequences backfill as 027-create-wishlists.ts: this table is
  // created after 022-create-id-sequences.ts ran, so it needs its own seed row.
